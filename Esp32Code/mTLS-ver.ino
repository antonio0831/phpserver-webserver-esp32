// this version incooperates mTLS as a method of authenication in addition to api key
#include <WiFi.h>
#include <WebServer.h>
#include <DHT.h>
#include <SPIFFS.h>
...
// your server cert root CA here
const char* rootCACertificate = R"(
-----BEGIN CERTIFICATE-----
certificate here (eg: ISRG X1)
-----END CERTIFICATE-----
)";

// the client certificate used for mTLS. You don't have to use the approach given (some people only need the leaf)
...
void sendDataToServer() {
  if (WiFi.status() == WL_CONNECTED) { // Check if WiFi is connected
    ...
    client.setCertificate(clientCertificate);    // ESP32 client certificate
    client.setPrivateKey(clientPrivateKey);      // ESP32 client private key
